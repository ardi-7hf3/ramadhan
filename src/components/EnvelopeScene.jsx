...
const EnvelopeScene = ({ t }) => {
  ...
  // peek: 0=hidden, 1=rising, 2=paused, 3=flying-up
  ...
  const handleOpen = useCallback(() => {
    ...
    // kartu berhenti di posisi setengah keluar — pause
    after(() => setPeekStep(2),       750 + 700)
    // kartu terbang ke atas (modal muncul)
    after(() => {
      setPeekStep(3)
      setCardPhase('opening')
      setEnvState('open')
    }, 750 + 700 + 600)
    // modal settled, peek card hilang
    after(() => {
      setPeekStep(0)
      setCardPhase('open')
    }, 750 + 700 + 600 + 600)
  ...
  // Posisi vertikal kartu peek:
  // step 0 → tersembunyi di dalam amplop (translateY 100%)
  // step 1 → naik setengah keluar (translateY -45%)  ← animasi naik
  // step 2 → diam di posisi setengah                  ← pause
  // step 3 → terbang ke atas (translateY -200%)        ← animasi naik lagi
  ...
  const peekTransition =
    peekStep === 1 ? 'transform 0.7s cubic-bezier(0.22,1,0.36,1), opacity 0.3s ease' :
    peekStep === 2 ? 'none' :
    peekStep === 3 ? 'transform 0.5s cubic-bezier(0.4,0,1,1), opacity 0.2s ease 0.1s' :
    'none'
  ...
  return (
    <>
      ...
      {/*
        STRUKTUR PENTING:
        - outerWrap: position relative, overflow VISIBLE agar kartu bisa keluar ke atas
        - envelope body: z-index 10  (di atas kartu peek)
        - peek card:     z-index 5   (di belakang amplop bagian bawah, tapi keliatan di atas)
        Kartu peek diposisikan absolute dari outerWrap, bottom=0
        translateY negatif → naik ke atas amplop
      */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 'clamp(280px, 75vw, 420px)' }}>

        {/* Hint */}
        {isClickable && (
          <div className="hint-bounce" style={{ marginBottom: '20px' }}>
            <div className="flex items-center gap-2.5 font-poppins font-medium text-sm px-5 py-2.5 rounded-full"
              style={{ background: 'rgba(201,148,26,0.08)', border: '1px solid rgba(201,148,26,0.22)', color: '#F5D87A' }}>
        ...
        {/* Outer wrapper — relative, overflow visible ke atas */}
        <div
          className={isClickable ? 'envelope-hover cursor-pointer' : 'cursor-default'}
          onClick={isClickable ? handleOpen : undefined}
          style={{
            position: 'relative',
            width:    '100%',
            // padding atas untuk ruang kartu peek naik
            paddingTop: '60px',
          }}
        >

          {/* ── PEEK CARD — absolute, naik ke atas amplop ── */}
          {peekStep > 0 && (
            <div
              style={{
                position:   'absolute',
            ...
            >
              ...
              <div style={{
                ...
                {/* corner ornaments */}
                <div style={{ position: 'absolute', top: 10, left: 10, width: 24, height: 24 }}>
                ...
                <div style={{ position: 'absolute', top: 10, right: 10, width: 24, height: 24 }}>
                  <svg width="24" height="24" viewBox="0 0 40 40" fill="none" style={{ transform: 'scaleX(-1)' }}>
                    <path d="M4 36 L4 4 L36 4" stroke="rgba(201,148,26,0.5)" strokeWidth="1.5" strokeLinecap="round"/>
                    <circle cx="4" cy="4" r="2.5" fill="rgba(201,148,26,0.5)"/>
                  </svg>
                </div>
          ...
          {/* ── ENVELOPE BODY — z-index lebih tinggi dari peek card ── */}
          <div
            style={{
              position:    'relative',
              zIndex:      10,
          ...
          >
            ...
            {/* Label */}
            <div className="absolute bottom-4 left-0 right-0 flex justify-center z-10 pointer-events-none" style={{
